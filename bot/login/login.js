...
let Canvas; try { Canvas = defaultRequire("canvas"); } catch (e) { Canvas = { createCanvas: () => ({ getContext: () => ({}) }), loadImage: async () => ({}) }; }
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			if (global.GoatBot.config.serverUptime.enable == true && !global.GoatBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
				...
				try {
					...
					try { await server.listen(PORT); } catch(e) { console.warn("[UPTIME] Port " + PORT + " in use, uptime server skipped."); }
