...
module.exports = {
  config: {
    ...
    category: "أوامر عامة",
    cooldown: 10,
    guide: "trash2 [mention-reply-UID]",
  },
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      const url = `${await baseApiUrl()}/api/dig?type=trash2&user=${id2}`;
      const img = await axios.get(url, { responseType: "arraybuffer" });
      const file = path.join(__dirname, `trash2_${id2}.png`);
      fs.writeFileSync(file, img.data);

      api.sendMessage({
        body: "Effect trash2 successful 🗑️",
